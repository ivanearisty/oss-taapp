...
class ServiceClientAdapter(Client):
    ...
    def __init__(self, service_client: AuthenticatedClient) -> None:
    ...
    def get_message(self, message_id: str) -> Message:
        ...
        """
        try:
            response = get_message_sync(message_id=message_id, client=self._service_client)
    ...
    def delete_message(self, message_id: str) -> bool:
        ...
        try:
            ...
            response = delete_message_sync(message_id=message_id, client=self._service_client)

            # Check if the response indicates success
            if response is None:
                self.logger.warning("Received None response when deleting message %s", message_id)
                return False

            # The response should contain success information
            success = response.get("success", True)  # Assume success if we got a response
            if not success:
                self.logger.warning("Delete operation reported failure for message %s", message_id)
                return False
    ...
    def mark_as_read(self, message_id: str) -> bool:
        ...
        try:
            ...
            response = mark_as_read_sync(message_id=message_id, client=self._service_client)

            # Check if the response indicates success
            if response is None:
                self.logger.warning("Received None response when marking message %s as read", message_id)
                return False

            # The response should contain success information
            success = response.get("success", True)  # Assume success if we got a response
            if not success:
                self.logger.warning("Mark as read operation reported failure for message %s", message_id)
                return False
    ...
    def get_messages(self, max_results: int = 10) -> Iterator[Message]:
        ...
        try:
            ...
            if not isinstance(response, list):
                self.logger.error("Expected list response but got %s", type(response).__name__)
                return
            ...
            for i, message_item in enumerate(messages_to_process):
                try:
                    ...
                    self.logger.warning("Failed to create ServiceMessage for item %d: %s", i, e)
