...
def test_get_kwargs_builds_correct_url_and_method():
    kw = mark_message_as_read._get_kwargs(message_id="abc")
    assert kw["method"] == "post"
    assert kw["url"] == "/messages/abc/mark-as-read"


def test_parse_200_and_sync_detailed():
    resp = httpx.Response(200, json={"status": "ok"})
    client = DummySyncClient(resp)
    detailed = mark_message_as_read.sync_detailed(message_id="abc", client=client)
    assert detailed.status_code == HTTPStatus.OK
    assert detailed.parsed == {"status": "ok"}

    parsed = mark_message_as_read.sync(message_id="abc", client=client)
    assert parsed == {"status": "ok"}


def test_parse_422_returns_validation_error():
...
def test_unexpected_status_raises_when_flag_true():
    ...
    with pytest.raises(errors.UnexpectedStatus):
        mark_message_as_read.sync_detailed(message_id="abc", client=client)


def test_unexpected_status_returns_none_when_flag_false():
    resp = httpx.Response(500, content=b"err")
...
def test_asyncio_parsing_and_build_response():
    resp = httpx.Response(200, json={"status": "async-ok"})
    client = DummyAsyncClient(resp)
    detailed = asyncio.run(mark_message_as_read.asyncio_detailed(message_id="abc", client=client))
    assert detailed.status_code == HTTPStatus.OK
    assert detailed.parsed == {"status": "async-ok"}


def test_request_kwargs_are_forwarded_to_client():
    resp = httpx.Response(200, json={"status": "ok"})
    ...
    parsed = mark_message_as_read.sync(message_id="xyz", client=client)
    assert parsed == {"status": "ok"}
