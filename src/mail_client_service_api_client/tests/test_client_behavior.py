...
def test_get_httpx_client_constructs_and_reuses():
...
def test_set_httpx_client_overrides_and_preserves():
    c = Client(base_url="http://example")
...
def test_with_headers_updates_existing_httpx_client_and_returns_new_instance():
    c = Client(base_url="http://example")
...
def test_with_cookies_updates_existing_httpx_client_and_returns_new_instance():
    c = Client(base_url="http://example")
    underlying = c.get_httpx_client()
...
def test_with_timeout_updates_underlying_client_and_returns_new_instance():
    c = Client(base_url="http://example")
    underlying = c.get_httpx_client()
    t = httpx.Timeout(5.0)
...
def test_context_manager_sync_enter_exit_no_error():
    c = Client(base_url="http://example")
    with c as ctx:
        assert ctx is c
...
def test_get_async_httpx_client_and_close():
    c = Client(base_url="http://example")
...
async def async_get_async_client(client: Client):
    return client.get_async_httpx_client()


def test_authenticated_client_injects_bearer_header_default():
...
def test_authenticated_client_custom_prefix_and_header_name():
...
def test_authenticated_client_async_injects_header_and_closure():
...
async def async_get_async_client_auth(ac: AuthenticatedClient):
...
def test_authenticated_client_header_variants(prefix, header_name, expected):
...
def test_with_headers_without_existing_clients_returns_new_instance_and_preserves_headers():
    c = Client(base_url="http://example")
...
def test_set_async_httpx_client_and_get_async_returns_provided_client():
    c = Client(base_url="http://example")
...
def test_with_headers_updates_existing_async_client_and_returns_new_instance():
    c = Client(base_url="http://example")
    async_client = asyncio.run(async_get_async_client(c))
...
def test_authenticated_client_context_manager_enter_exit_and_set_async_client_behaviour():
...
def test_with_timeout_updates_existing_async_client_and_returns_new_instance():
    c = Client(base_url="http://example")
    async_client = asyncio.run(async_get_async_client(c))
    t = httpx.Timeout(7.0)
    new = c.with_timeout(t)
