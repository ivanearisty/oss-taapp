...
def test_get_kwargs_includes_and_excludes_interactive():
...
def test_sync_detailed_and_sync_parse_200():
    resp = httpx.Response(200, json={"ok": True})
    client = DummySyncClient(resp)
    detailed = login.sync_detailed(client=client)
    assert detailed.status_code == HTTPStatus.OK
    assert detailed.parsed == {"ok": True}

    parsed = login.sync(client=client)
    assert parsed == {"ok": True}


def test_sync_parse_422_returns_validation_error():
...
def test_sync_detailed_unexpected_status_raises():
    ...
    with pytest.raises(errors.UnexpectedStatus):
        login.sync_detailed(client=client)


def test_asyncio_detailed_and_async_parse_200():
    resp = httpx.Response(200, json={"ok": "async"})
    client = DummyAsyncClient(resp)
    detailed = asyncio.run(login.asyncio_detailed(client=client))
    assert detailed.status_code == HTTPStatus.OK
    assert detailed.parsed == {"ok": "async"}

    parsed = asyncio.run(login.asyncio(client=client))
    assert parsed == {"ok": "async"}

# ...existing code...
def test_get_kwargs_interactive_true():
    kw = login._get_kwargs(interactive=True)
    assert "params" in kw
    assert kw["params"]["interactive"] is True

def test_get_kwargs_interactive_none_removes_param():
    kw = login._get_kwargs(interactive=None)
    assert "params" in kw
    assert kw["params"] == {}

def test_sync_detailed_unexpected_status_returns_none_when_flag_false():
...
def test_asyncio_detailed_unexpected_status_raises():
    resp = httpx.Response(503, content=b"unavailable")
    client = DummyAsyncClient(resp, raise_on_unexpected_status=True)
    with pytest.raises(errors.UnexpectedStatus):
        asyncio.run(login.asyncio_detailed(client=client))

def test_request_kwargs_forwarded_to_client():
    resp = httpx.Response(200, json={"ok": True})
    ...
    parsed = login.sync(client=client, interactive=True)
    assert parsed == {"ok": True}
# ...existing code...